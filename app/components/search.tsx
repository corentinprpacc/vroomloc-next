    ...
    return (
        <div className="bg-black">
            <ul>
                {
                    cities.map((city, id) => {
                        return <li key={id} className="text-white">{city}</li>
                    })
                }
            </ul>
            <form onSubmit={handleSubmit(onSubmit)} className="container sm:max-md:pt-6 md:flex md:justify-between">
                <button className="w-full md:mt-6 md:w-auto text-white flex justify-center border border-solid border-white/50 hover:border-white py-4 px-8">
                    <span className="mr-3">
                        <Image src="/images/icons/filter-icon.svg" alt="Filter" width={22} height={22}></Image>
                    </span>
                    <span>Filtrer</span>
                </button>
                <div className="mt-6 md:w-1/2 border border-solid border-white/50 hover:border-white relative">
                    ...
                    {
                        cityInput && (
                            <button onClick={handleRemoveSearch} type="button" title="remove search" className="absolute right-10 top-5">
                                <Image src="/images/icons/remove-icon.svg" alt="Remove search" width={16} height={16}></Image>
                            </button>
                        )
                    }
                    <datalist id="cars">
                ...
                <div className="relative z-0">
                    <label className="absolute z-10 top-4 left-4 px-2 text-[#c2c2c2] text-xs bg-black">Départ</label>
                    <Controller
                        control={control}
                ...
                <div className="relative">
                    <label className="absolute z-10 top-4 left-4 px-2 text-[#c2c2c2] text-xs bg-black">Retour</label>
                    <Controller
                        control={control}
